...
void FbxParts::InitMaterial(fbxsdk::FbxNode* pNode)
{
	...
	for (DWORD i = 0; i < materialCount_; i++)
	{
		...
		// デフォルト値
		pMaterial_[i].ambient = XMFLOAT4(0, 0, 0, 1);
		pMaterial_[i].diffuse = XMFLOAT4(1, 1, 1, 1);
		pMaterial_[i].specular = XMFLOAT4(0, 0, 0, 1);
		pMaterial_[i].shininess = 0;

		// Phongか確認してからキャスト
		if (pMaterial->GetClassId().Is(FbxSurfacePhong::ClassId))
		{
			FbxSurfacePhong* pPhong =
				static_cast<FbxSurfacePhong*>(pMaterial);
			...
			pMaterial_[i].shininess =
				(float)pPhong->Shininess;
		}
		// Lambertの場合
